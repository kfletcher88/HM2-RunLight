...

{
	...
	while(<$inFH>){
		next if $_ !~ m/^.CLK\n/; #.=}
		if($_ =~ m/co1:(\S+)\nco2:(\S+)\n.+ovt:(\S+)\n.+num:(\S+)\nsta:(\S+)\n/s){
			#next if $5 eq 'B' or $5 eq 'C'; # could be Bad, Chimera, Assembly, Unknown
			my $ovt= $3 eq 'O' ? 1 : 0;
			my $num= $4;
			#die "$1 e $2\n" if $1 eq $2; #testing#
			if(exists($clk{$1.'@'.$2})){
...

{
	...
	# for 22:tsc_space_ali 23:tsc_space_hang 24:tsc_N	
		...
		for(my $k=0;$k<=$ctgrr;$k++){
			push @ctg5,$agp{$tsc}->[$k][5] if $agp{$tsc}->[$k][4] eq 'W' and $agp{$tsc}->[$k][2]>=$tsc_point-$range;
		}
		for(my $k=$ctgrr+1;$k<$tsc_count;$k++){
			push @ctg3,$agp{$tsc}->[$k][5] if $agp{$tsc}->[$k][4] eq 'W' and $agp{$tsc}->[$k][1]<=$tsc_point+$range;
		}
		print STDERR "target: @ctg5 \ntarget:@ctg3\n";
		for(my $i=0;$i<scalar(@ctg5);$i++){
			for(my $k=0;$k<scalar(@ctg3);$k++){
				if(exists $clk{$ctg5[$i].'@'.$ctg3[$k]}){ 
					$num+=$clk{$ctg5[$i].'@'.$ctg3[$k]}->[1]; 
					print STDERR "  target $tsc $tsc_point at $agp{$tsc}->[$tsc_it][5] : $ctg5[$i] - $ctg3[$k] : ".$clk{$ctg5[$i].'@'.$ctg3[$k]}->[1]."\n"; #testing#
				}
				if(exists $clk{$ctg3[$k].'@'.$ctg5[$i]}){ 
					$num+=$clk{$ctg3[$k].'@'.$ctg5[$i]}->[1];
					print STDERR "  target $tsc $tsc_point at $agp{$tsc}->[$tsc_it][5] : $ctg3[$k] - $ctg5[$i] : ".$clk{$ctg3[$k].'@'.$ctg5[$i]}->[1]."\n"; #testing#
				}
			}
		}
		$tt->[21]=$num;
		...
		for(my $k=0;$k<=$ctgrr;$k++){
			push @ctg5,$agp{$qsc}->[$k][5] if $agp{$qsc}->[$k][4] eq 'W' and $agp{$qsc}->[$k][2]>=$qsc_point-$range;
		}
		for(my $k=$ctgrr+1;$k<$qsc_count;$k++){
			push @ctg3,$agp{$qsc}->[$k][5] if $agp{$qsc}->[$k][4] eq 'W' and $agp{$qsc}->[$k][1]<=$qsc_point+$range;
		}
		my @ttt=(@ctg5,@ctg3);
		if($strand>0){  #####
		...
		for(my $i=0;$i<scalar(@ctg5);$i++){
			for(my $k=0;$k<scalar(@ctg3);$k++){
				if(exists $clk{$ctg5[$i].'@'.$ctg3[$k]}){ 
					$num+=$clk{$ctg5[$i].'@'.$ctg3[$k]}->[1];
					print STDERR "  query $qsc $qsc_point at $agp{$qsc}->[$qsc_it][5] : $ctg5[$i] - $ctg3[$k] : ".$clk{$ctg5[$i].'@'.$ctg3[$k]}->[1]."\n"; #testing#
				}
				if(exists $clk{$ctg3[$k].'@'.$ctg5[$i]}){
					$num+=$clk{$ctg3[$k].'@'.$ctg5[$i]}->[1];
					print STDERR "  query $qsc $qsc_point at $agp{$qsc}->[$qsc_it][5] : $ctg3[$k] - $ctg5[$i] : ".$clk{$ctg3[$k].'@'.$ctg5[$i]}->[1]."\n"; #testing#				
				}
			}
		}
		$tt->[25]=$num;	
		...
		##################	决定对tsc还是qsc进行切断 ################	
		if($tt->[21]>1.6*$tt->[25] and $tt->[27]-$tt->[23]<1000){
			$tt->[20]=2;
		}elsif($tt->[25]>1.6*$tt->[21] and $tt->[23]-$tt->[27]<1000){
			$tt->[20]=1;
