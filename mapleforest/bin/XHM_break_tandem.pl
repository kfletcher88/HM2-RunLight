...
if (@ARGV < 3 || $Arg_list =~ m/--help|-\?/) { print <<'USAGES' and exit }
   ...
   NOTE that no need to change prefix of the scf_id.
   Used for non-CA6.1 scf and agp files.
...

{
	...
	while(<$inFH>){
		...
		chomp;
		my @tt =  split /\t/,$_;
		# $tt[0]=~s/scf//;  ### only use for CA6.1, where scf_id in agp has not prefix "scf" but scf_id in scf.fa has
		$tt[2]+=$tt[1]; $tt[4]+=$tt[3];
		push @tt,0;
		push @tandems,\@tt;
	}
