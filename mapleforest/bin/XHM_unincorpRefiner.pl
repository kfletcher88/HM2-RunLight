...
if($para{"--runLastzChainNet"}>0){
	...
	$cmd = "../bin/initiation.pl --faSplit --faToNib --faSize --Species unpaired optiNewScaffolds --Force --Delete";
	$cmd.= " 1>_un_initiation.log 2>>_un_initiation.log";
	system($cmd);
	print "Finish ",$cmd,"\n";
	
	$cmd = "../bin/HM_all_lastz_mThreads.pl --Species unpaired optiNewScaffolds  --threads=" . $para{"--threads"};
	$cmd.= " --targetSize=10000000 --identity=" . $para{'--identity'};
	$cmd.= " --Force --Delete 1>_un_all_lastz.log 2>>_un_all_lastz.log";
	system($cmd);
	print "Finish ",$cmd,"\n"; 

	$cmd = "../bin/HM_axtChainRecipBestNet.pl --rbestNet --axtChain --tbest --threads=" . $para{"--threads"};
